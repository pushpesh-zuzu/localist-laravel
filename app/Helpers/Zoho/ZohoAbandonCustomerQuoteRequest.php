...
if (empty($questions)) {
    ...
    protected function buildAbandonQuoteRequestPayload($accessToken, $userId)
    {
        ...
        $service=Category::find($user->service_id)->name;



$creditScore=0;
           $creditScoreModel = Category::where('id',$user->service_id)->value('credit_score_model');
    ...
    // Build formatted Q&A text block
        ...
        return [
            'data' => [[
                'Quote_Request_Record_Id'      => (int) ($user->id . rand(100, 999)),
   ...
   public function deleteAbandonedQuoteRequest($zohoAbandonedQuoteId, $abUserId)
{
    ...
    if ($response->successful()) {
        Log::info("✅ Zoho Abandoned Quote Request deleted successfully.", [
            'user_id' => $abUserId,
