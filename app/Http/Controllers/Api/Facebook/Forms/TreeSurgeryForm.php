...
class TreeSurgeryForm extends Controller
{
    
    public function getFacebookLeadsTreeSurgeryForm(Request $request, LeadService $leadService){
        $payload   = $request->all();
        $serviceId = 112;
    ...
    private function getFacebookQuestionsInfoArray($payload, $serviceId)
{
    ...
    //remove blank answer array from $questions
    $cleanedQuestions = $filtered = array_values(array_filter($questions, function ($item) {
        return isset($item['ans']) && trim($item['ans']) !== '';
    }));
